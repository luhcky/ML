...
# ── Load pipeline ─────────────────────────────────────────
pipeline  = joblib.load(f'{MODEL_DIR}/attrition_pipe.pkl')
...
try:
    ...
    EXPECTED_VALUE = 1 / (1 + math.exp(-raw_ev)) if abs(raw_ev) > 1 else raw_ev
    logger.info(f"SHAP loaded. Base value={EXPECTED_VALUE:.4f} ({EXPECTED_VALUE*100:.2f}%)")
except ImportError:
    SHAP_AVAILABLE = False
    EXPECTED_VALUE = 0.161
...
app = FastAPI(
    ...
    description=(
        "XGBoost model with SHAP explainability and GenAI HR reports.\n\n"
        "**Endpoints:**\n"
        "- `POST /predict` — attrition score + risk tier\n"
        "- `POST /predict/explain` — full SHAP values\n"
        "- `POST /predict/report` — prediction + AI-written HR advisory\n"
        "- `POST /predict/batch` — score up to 1000 employees\n"
...
class EmployeeInput(BaseModel):
    ...
    def v_travel(cls, v):
        ...
        if v not in valid:
            raise ValueError(f"BusinessTravel must be one of {valid}")
        return v
...
def generate_template_report(prob, tier, risk_factors,
                              protective_factors, top_driver, emp) -> str:
    """Structured fallback report when Claude API not available."""
    urgency = {
        "CRITICAL": "immediate — act within 24 hours",
        "HIGH"    : "high — act within this week",
        "MEDIUM"  : "moderate — act within 30 days",
        "LOW"     : "low — routine monitoring sufficient",
    }.get(tier, "moderate")

    p1 = (
        f"This employee presents a {tier.lower()} attrition risk with a predicted "
        f"probability of {prob*100:.1f}%, placing them "
        f"{'significantly above' if prob > 0.3 else 'above'} the organisational "
        f"baseline of 16.1%. The urgency level is {urgency}. "
        f"The primary model driver is {top_driver.replace('_',' ')}, which was "
        f"identified as the strongest predictor of departure for this profile."
    )

    risk_count = len(risk_factors)
    prot_count = len(protective_factors)
    p2 = (
        f"The profile shows {risk_count} active risk signal"
        f"{'s' if risk_count != 1 else ''} against "
        f"{prot_count} protective factor{'s' if prot_count != 1 else ''}. "
        f"The pattern — {', '.join(risk_factors[:2]).lower()} — is consistent "
        f"with an employee who is disengaged and actively weighing their options. "
        f"With limited protective anchors in place, the risk of voluntary "
        f"departure within 90 days is elevated."
    )

    actions = []
    if any("overtime" in r.lower()    for r in risk_factors):
        actions.append("schedule a workload review with the line manager this week")
    if any("satisfaction" in r.lower() for r in risk_factors):
        actions.append("conduct a confidential 1:1 career conversation focusing on role fulfilment")
    if any("income" in r.lower() or "stock" in r.lower() for r in risk_factors):
        actions.append("commission a market salary benchmarking exercise and review equity eligibility")
    if any("promotion" in r.lower()   for r in risk_factors):
        actions.append("discuss a clear and time-bound promotion roadmap with specific milestones")
    if not actions:
        actions = [
            "schedule a career development conversation this week",
            "review compensation against market benchmarks",
            "assess workload and overtime patterns for the past 90 days",
        ]

    p3 = (
        f"Recommended actions: first, {actions[0]}. "
        f"Second, {actions[1] if len(actions)>1 else 'monitor engagement signals weekly'}. "
        f"Third, {actions[2] if len(actions)>2 else 'document all interventions for tracking'}. "
        f"If no action is taken within 30 days, historical data suggests the "
        f"probability of resignation rises above 85% within 90 days."
    )
    return f"{p1}\n\n{p2}\n\n{p3}"
...
def predict_explain(employee: EmployeeInput):
    ...
    try:
        ...
        # Convert SHAP from log-odds to probability space
        scale_factor = prob * (1 - prob)
        sv_prob      = sv_row * scale_factor

        shap_dict = {
            feat: round(float(val), 6)
            for feat, val in zip(FEATURES, sv_prob)
        }
        ...
        top_drivers = [
            {
                "feature"   : feat,
                "shap_value": val,
                "direction" : "increases attrition risk" if val > 0
        ...
        for feat, val in sorted_shap[:3]:
            ...
            parts.append(
                f"{feat} {direction} attrition probability "
        ...
        return {
            ...
            "protective_factors"   : get_protective_factors(employee),
            "shap_base_value"      : round(EXPECTED_VALUE, 4),
            "shap_base_pct"        : round(EXPECTED_VALUE * 100, 2),
            "shap_values"          : shap_dict,
            "shap_top_drivers"     : top_drivers,
            "shap_explanation"     : shap_explanation,
            "shap_note"            : (
                f"Base attrition rate across all employees: "
                f"{EXPECTED_VALUE*100:.1f}%. "
                f"SHAP values show how each feature moved this employee's "
...
def predict_with_report(employee: EmployeeInput):
    start = time.time()
    ...
    # Step 2 — top SHAP driver
    top_driver = "OverTime"  # fallback
    if SHAP_AVAILABLE:
        try:
            X_df      = pd.DataFrame(X_s, columns=FEATURES)
            shap_vals = EXPLAINER.shap_values(X_df)
            sv        = shap_vals[1] if isinstance(shap_vals, list) else shap_vals
            sv_row    = sv[0]
            top_idx   = int(abs(sv_row).argmax())
            top_driver = FEATURES[top_idx]
        except Exception as e:
            logger.warning(f"SHAP for report failed: {e}")
    ...
    if api_key and ANTHROPIC_AVAILABLE:
        try:
            ...
            prompt = f"""You are a senior HR analytics consultant writing a
confidential advisory report for an HR manager.

PREDICTION RESULTS:
- Risk Tier: {tier}
- Attrition Probability: {prob*100:.1f}%
- Top Model Driver: {top_driver.replace('_',' ')}

RISK SIGNALS:
{chr(10).join(f'- {r}' for r in risk_factors)}

PROTECTIVE FACTORS:
{chr(10).join(f'- {p}' for p in protective_factors)}

EMPLOYEE SUMMARY:
- Age: {employee.Age} | Income: ${employee.MonthlyIncome:,}/month
- Overtime: {employee.OverTime} | Job Level: {employee.JobLevel}/5
- Job Satisfaction: {employee.JobSatisfaction}/4
- Stock Options: {employee.StockOptionLevel}/3
- Years at Company: {employee.YearsAtCompany}
- Years Since Promotion: {employee.YearsSinceLastPromotion}
- Marital Status: {employee.MaritalStatus}
- Travel: {employee.BusinessTravel}

Write a professional HR advisory report with exactly 3 paragraphs:

Paragraph 1 — RISK ASSESSMENT:
State the risk level, urgency, and single most important driver.
Include the actual probability number. No bullet points.

Paragraph 2 — ROOT CAUSE ANALYSIS:
Explain what the combination of signals means for this specific employee.
What is their likely experience and mindset right now?
Be empathetic and insightful — not clinical.

Paragraph 3 — RECOMMENDED ACTIONS:
Give 3 specific concrete actions for HR this week.
Tie each action to a specific risk signal.
End with what happens if no action is taken in 30 days.

Tone: Professional, direct, empathetic. No bullet points. No headers.
Length: 250-350 words."""
            ...
            ai_source  = "anthropic_api"

        except Exception as e:
            logger.error(f"Claude API error: {e}")
            ai_report = generate_template_report(
                prob, tier, risk_factors,
                protective_factors, top_driver, employee
            )
            ai_model  = "template_fallback"
            ai_source = f"fallback — Claude error: {str(e)[:100]}"
    else:
        ai_report = generate_template_report(
            prob, tier, risk_factors,
            protective_factors, top_driver, employee
...
def predict_batch(employees: List[EmployeeInput]):
    if len(employees) > 1000:
        raise HTTPException(status_code=422,
                            detail="Max 1000 employees per batch")
    start   = time.time()
    results, flagged = [], 0
